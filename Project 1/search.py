...
def depthFirstSearch(problem):
    ...
    while not stack.isEmpty():
        ...
        if problem.isGoalState(curr_node[0]):
            while curr_node[0] != problem.getStartState():
                path.insert(0,curr_node[1])
                curr_node=parent[curr_node]
            return path
        visited.append(curr_node[0])
        for i in problem.getSuccessors(curr_node[0]):
...
def breadthFirstSearch(problem):
    ...
    while not queue.isEmpty():
        curr_node=queue.pop()
        if curr_node[0] in visited:continue
        if problem.isGoalState(curr_node[0]):
            while curr_node[0] != problem.getStartState():
                path.insert(0,curr_node[1])
                curr_node=parent[curr_node]
...
def uniformCostSearch(problem):
    ...
    while not queue.isEmpty():
        ...
        for i in problem.getSuccessors(curr_node[0]):
            if i[0] in visited: continue
            cost[i[0]] = cost[curr_node[0]] + i[2]
            queue.update(i,cost[i[0]])
            parent[i] = curr_node
...
def aStarSearch(problem, heuristic = nullHeuristic):
    ...
    while not queue.isEmpty():
        ...
        for i in problem.getSuccessors(curr_node[0]):
            if i[0] in visited: continue
            cost[i[0]] = cost[curr_node[0]] + i[2]
            queue.update(i, cost[i[0]] + heuristic(i[0],problem))
            parent[i] = curr_node
